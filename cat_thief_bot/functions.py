...
def get_string_photo(wall_id, num_photos):
    i = 0
    photo = ''
    data1 = get_posts(wall_id)
    while i < num_photos:
        photo = photo + f"photo{data1['items'][0]['attachments'][i]['photo']['owner_id']}_{data1['items'][0]['attachments'][i]['photo']['id']}"
        if i + 1 != num_photos:
            photo = photo + ','
            i = i + 1
        elif i + 1 == num_photos:
...
def parser(group_name='', owner_id=0, date=0):
    ...
    if date_of_post > date:
        ...
        # next_date = date_of_post
        if flag == 0:
            if info[0]['type'] == 'photo':
                num_photos = len(data['items'][0]['attachments'])
                if num_photos > 6:
                    num_photos = 5
                photo = get_string_photo(wall_id, num_photos)

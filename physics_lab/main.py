...
def main():
    ...
    arr_t = []
    print("Enter speed: ")
    speed = float(get_number())
    print("Enter angle: ")
    angle = float(get_number())
    print("Enter resistance")
    coef = float(get_number())
    option = 0
    if coef == 0:
        option = 2
    elif coef > 0:
    ...
    if option == 1:
        ...
        if choice == 1:
            ...
            draw_func_flying_point2(arr_x, arr_y, '$$y=f(x)$$', "Ballistic trajectory with resistance",
        ...
        if choice == 1:
            ...
            draw_func_flying_point2(arr_x, arr_y, '$$y(x)=x*tg(a)-\\frac{g*x^2}{2*v^2*cos(a)^2*t}$$',
                                    "Ballistic trajectory", "Length of flight X", "Height of flight Y")
